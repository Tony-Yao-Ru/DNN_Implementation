...
class DeepNeuralNetwork:
    ...
    def Forward_propagation(self, input):
        if input.ndim == 1:
            ...
            input = input.T  # shape: (features, batch_size)
        ...
        for i in range(1, self.layers):
            ...
            if i == self.layers - 1:
                if self.nodes[i] == 1:
                    self.cache[f"z_{i}"] = self.sigmoid(a)
                else:
                    self.cache[f"z_{i}"] = self.softmax(a)
            else:
                self.cache[f"z_{i}"] = self.Activation(a)

        return self.cache[f"z_{self.layers - 1}"].T  # shape: (batch_size, output_dim)
    ...
    def Backward_propagation(self, Y):
        ...
        if Y.ndim == 1:
            ...
            Y = Y.T  # shape: (classes, batch_size)
    ...
    def cross_entropy_loss(self, y, output):
        ...
        log_probs = np.multiply(y, np.log(output))
